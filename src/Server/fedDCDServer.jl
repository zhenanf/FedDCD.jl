...
########################################################################################################
mutable struct AccFedDCDServerNN{T1<:Int64, T2<:Float64, T3<:SparseMatrixCSC{Float64, Int64}, T4<:Vector{Int64}, T5<:Vector{AccFedDCDClientNN}, T6<:Flux.Chain, T7<:Flux.OneHotArray} <:AbstractServer
    Xtest::T3                       # testing data
    XtestT::T3                      # copy of transpose
    Ytest::T7                       # testing label
    num_classes::T1                 # number of classes
    ...
    selectedIndices::T4             # selected clients
    η::T2                           # learning rate                    
    function AccFedDCDServerNN(Xtest::SparseMatrixCSC{Float64, Int64}, Ytest::Vector{Int64}, dim::Int64, config::Dict{String, Real})
        num_classes = config["num_classes"]
        num_clients = config["num_clients"]
        participation_rate = config["participation_rate"]
        η = config["learning_rate"]
        ...
        XtestT = copy(Xtest')
        Ytest = Flux.onehotbatch(Ytest, 1:num_classes)
        clients = Vector{AccFedDCDClientNN}(undef, num_clients)
        selectedIndices = zeros(Int64, τ)
        new{Int64, Float64, SparseMatrixCSC{Float64, Int64}, Vector{Int64}, Vector{AccFedDCDClientNN}, Flux.Chain, Flux.OneHotArray}(Xtest, XtestT, Ytest, num_classes, num_clients, participation_rate, clients, W, τ, selectedIndices, η)
    end
end

# Select clients
function select!(server::AccFedDCDServerNN)
    server.selectedIndices .= randperm(server.num_clients)[1:server.τ]
    return nothing
end

# Send global model
function sendModel!(server::AccFedDCDServerNN, round::Int64)
    # Only send model to selected clients
    Threads.@threads for i = 1:server.τ
        idx = server.selectedIndices[i]
        client = server.clients[idx]
        if round == 1
            for j = 1:length(client.y)
                client.y[j] .-= server.η * (params(client.W)[j] - params(server.W)[j])
            end
        else
            a = client.a; b = client.b; r = client.r
            θ = a*r / (a^2 + b)
            for j = 1:length(client.z)
                client.z[j] .-= server.η * θ * (params(client.W)[j] - params(server.W)[j])
            end
        end
    end
    return nothing
...
# synchronization
function synchronize!(server::AccFedDCDServerNN)
    for i in server.selectedIndices
        c = server.clients[i]
        for j = 1:length(params(server.W))
            params(c.W)[j] .= params(server.W)[j]
...
# Calculate objective value
function getObjValue(
    server::Union{FedDCDServerNN, AccFedDCDServerNN}
)
    objValue = 0.0
    totalNumData = 0
    for i = 1:server.num_clients
